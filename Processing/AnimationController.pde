...
class AnimationController {
  ...
  int spriteDirection = 0;
    ...
    public void displayIdle(int direction, int x, int y) {
      image(playerSpritesIdle[direction], x, y); 
    }
    public void displayAnimated(int direction, int x, int y) {
      playerSpritesAnimated[direction].display( x, y);
    }
}
